...
import { Avatar, Button, Card, Col, Divider, Layout, Radio, Row, Select, Space, Spin, Typography } from "antd";
...
const Dashboard = () => {
    ...
    useEffect(() => {
        async function loadAircrafts(country) {
            const { states, time } = await getAircrafts(country);
            setLoading(false);
            setAircrafts(states);
    ...
    const { airports, countries } = ACME_DATA;
    return (
        <Content style={{ textAlign: 'left', backgroundColor: "#F6F8FC", padding: '20px' }}>
            ...
            <Divider style={{ margin: '10px 0' }} />
            <Typography>
                <Title level={4}>{country ? `Aircrafts group by : ${country}` : `All Aircrafts`}</Title>
            </Typography>
            {
                loading ? (
